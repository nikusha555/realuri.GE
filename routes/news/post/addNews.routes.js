...
router.post('/', async (req, res) => {
    const { title, content, imageFileName, isHot } = req.body;
    ...
    if (!title || !content || !imageFileName ) {
        return res.status(400).json({ error: 'All fields (title, content, imageFileName) are required' });
    }

    const query = `
            INSERT INTO news (title, content, image_file_name, is_hot, hot_until) 
            VALUES (?, ?, ?, ?, ?)
        `;

    try {
        // Execute the insert query and pass all necessary values, including is_hot and hot_until
        const [result] = await db.query(query, [title, content, imageFileName, isHot ? 1 : 0, hotUntil]);
